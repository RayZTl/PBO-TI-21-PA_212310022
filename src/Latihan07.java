...
public class Latihan07 {
	...
	public static void main(String[] args) {
		...
		int matriksA[][]= new int[baris][kolom]; int matriksB[][]= new int[baris][kolom]; int hasil1[][] = new int[baris][kolom];

		JOptionPane.showMessageDialog(null, "==== Matriks A ====");
		for(int i =0; i<matriksA.length; i++) {
		for(int j=0; j<matriksA.length; j++) { matriksA[i][j] =
		Integer.parseInt(JOptionPane.showInputDialog(null,"Masukkan angka: ","Matriks A ke-["+i+"]"+"["+j+"]",JOptionPane.QUESTION_MESSAGE));
		}
		}
		...
		for(int i=0; i<matriksA.length;i++) {
		for(int j=0;j<matriksA.length; j++) { outputA += matriksA[i][j] + " ";
		}
		outputA += "\n";
		}
		...
		for(int i=0; i<hasil1.length;i++) {
		for(int j=0;j<hasil1.length; j++) { output1 += hasil1[i][j] + " ";
		}
		output1 += "\n";
		}
